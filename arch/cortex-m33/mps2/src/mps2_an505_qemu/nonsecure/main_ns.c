/**
 * @file        arch/cortex-m33/mps2/src/mps2_an505_qemu/nonsecure/main_ns.c
...
extern void Secure_printf(int pointer);
...
#define UARTDR()    (*((volatile uint32_t *)(0x40200000 + 0x00)))
#define UARTCTRL()  (*((volatile uint32_t *)(0x40200000 + 0x08)))
#define UARTSTATE() (*((volatile uint32_t *)(0x40200000 + 0x04)))
...
/**
 * @brief         initUart0 - performs initialization of UART0.
 *
 * @param         None
 *
 * @returns       None
...
/**
 * @brief         _putchar - performs initialization of UART0.
 *
 * @param         ch - the char symbol to be sent
 *
 * @returns       None
 */
void _putchar(char ch)
...
/**
 * @brief         _getchar - performs initialization of UART0.
 *
 * @param         None
 *
 * @returns       ch - the received char 
 */
char _getchar(void)
{
  char ch;
...
void menu_security_exception_example_ns(void)
{
  ...
  printf("+----------------------------------------------------------------------+\n");
  printf("| FLASH | 0x%08X - 0x1007FFFF | 0x%08X | %dk                  |\n", CONFIG_START_ADDRESS_BL33, 0x10080000 - CONFIG_START_ADDRESS_BL33, (0x10080000 - CONFIG_START_ADDRESS_BL33)/1024);
...
//   printf("|     | Read SRAM non-secure address 0x%08X   | Hard fault         |\n",(~0x10000000 & (unsigned int)&temp));
  ...
  printf("| [6] | Read FLASH non-secure address 0x%08X  | Access successful  |\n", CONFIG_START_ADDRESS_BL33);
  printf("|     | Read FLASH non-secure address 0x%08X  | Hard fault         |\n", (CONFIG_START_ADDRESS_BL33 - 0x10000000));
...
//       printf("    Read FLASH non-secure address 0x%08X: Hard fault occurs in secure\n", (CONFIG_START_ADDRESS_BL33 - 0x10000000));
//       M32(0x00040000);
    //   break;
    // case '7':
