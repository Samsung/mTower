 ...
 */
static void truncate_(uint8_t *hmac_result, uint32_t *bin_code)
{
 ...
 ******************************************************************************/
TEE_Result HOTP_TA_CreateEntryPoint(void)
{
	return TEE_SUCCESS;
}

void HOTP_TA_DestroyEntryPoint(void)
{
}

TEE_Result HOTP_TA_OpenSessionEntryPoint(uint32_t param_types,
...
void HOTP_TA_CloseSessionEntryPoint(void __unused *sess_ctx)
{
}

TEE_Result HOTP_TA_InvokeCommandEntryPoint(void __unused *sess_ctx,
				      uint32_t cmd_id,
...
user_ta_register(.uuid = TA_HOTP_UUID, .name = "HOTP",
       .flags = TA_FLAG_USER_MODE,
       .open_session_entry_point = HOTP_TA_OpenSessionEntryPoint,
       .close_session_entry_point = HOTP_TA_CloseSessionEntryPoint,
       .invoke_command_entry_point = HOTP_TA_InvokeCommandEntryPoint);
